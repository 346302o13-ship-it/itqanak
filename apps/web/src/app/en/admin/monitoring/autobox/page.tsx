...
export const metadata = { title: "AutoBox events" };
...
export default async function EnglishAdminAutoboxPage({ searchParams }: PageProps) {
  const [principal, csrfToken, query] = await Promise.all([
    requireAdminPagePrincipal("/en/admin/monitoring/autobox", "en", "admin.operations.read"),
    csrfTokenForPage(),
    searchParams,
  ]);

...
export const metadata = { title: "الطلبات المعلّقة" };
...
export default async function AdminPendingRequestsPage({ searchParams }: PageProps) {
  const [principal, csrfToken, query] = await Promise.all([
    requireAdminPagePrincipal("/ar/admin/requests/pending"),
    csrfTokenForPage(),
    searchParams,
  ]);

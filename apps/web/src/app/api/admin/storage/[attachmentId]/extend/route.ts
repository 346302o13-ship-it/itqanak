...
export async function POST(
  ...
  try {
    ...
    const fallback = `/${locale}/admin/storage`;
    const { attachmentId } = await context.params;
    const rawDays = formValue(protectedForm.formData, "days");
    const days = /^\d{1,4}$/u.test(rawDays) ? Number(rawDays) : 30;
    ...
    try {
      ...
      await app.storageAdmin.extendRetention(principal, attachmentId, days, {
        ...protectedForm.context,
        requestId,
      });
      const destination = new URL(fallback, protectedForm.config.adminAppUrl);
      destination.searchParams.set("notice", "extended");
